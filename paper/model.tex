...
frame.%  \citep[see][for explicit expressions]{masset02}
In the momentum 
...
initialised close to steady state. The $\mathcal{C}$ function attempts to restore the initial energy density (and
...
The disc occupies $r\in[r_\mathrm{in}, r_\mathrm{out}]$ with
$r_\mathrm{out}=25\mathrm{in}$ {\bf check: if all simulations use the
  same domain, state it here} and $\phi\in[0,2\pi]$. The initial disc is axisymmetric with
...
pressure, stellar gravity and disc gravity. The initial radial velocity is $v_{r}=3\nu/r$ 
with $\nu = \hat{\nu}\rin^2\Omega_k(\rin)$. For parameter values employed in this work, $v_{\phi}\simeq r\Omega_k$ 
and $\hat{\nu}= 10^{-9}$, so that $|v_{r}/v_{\phi}|\ll1$ and the initial 
flow is effectively only in the azimuthal direction.  With this choice
of physical viscosity $\hat{\nu}$ the only heating source is through
shock-heating (via artificial viscosity) and the $\mathcal{C}$
function when $e\Sigma<e_i\Sigma_i$. 
...
\end{align}
where $x_s=2r_h$ is the approximate distance from the planet to its
...
taken for a gap edge fluid element to encounter planet-induced
shocks.   
...
The generalized potential vorticity is defined as
\begin{align}
...
The Rossby wave instability is characterized by exponetially
growing pertibations. Though in this paper we do not formally consider
linear instability, modal analysis will be useful to
analyse the growth of different wavenumbers, which is associated with
the number of vortices initially formed by the RWI.  

%characterize 
%azimuthal vortex numbers and overdensity values. 
...
\end{align} 
where $m$ is the aziumuthal wave number. We define the growth rate
...
over a raidal region of interest. By using Eq.~\ref{growth} the growth
...
% the ratio of interial to Corriolis forces. 
