...
{\bf For completeness we also performed a lower resolution simulation
  ...
  isothermal disk) by setting $\gamma=1.01$. This simulation
  yield a most unstable growth rate $\sigma/\Omega(r_p)\simeq 0.05$,
  compared with a value of $0.08$ for a case with
  $\tilde{\beta}=0.01$. However, the vortex evolutions were similar.}  
...
{\bf This decay is probably due to numerical viscosity. During the
  ...
  $\tilde{\beta}=0.1,\,0.5$ and $1.0$ we also observe the appearence of
...
high-resolution runs reported below. {\bf These simulations
  include a case with $\tilde{\beta}=0.01$ and a locally isothermal
  disk run, which show similar behavior. In particular, we find vortex
  lifetimes have a non-monotonic dependence on $\tilde{\beta}$, with
  longest lifetimes as $\tilde{\beta}\to 0$ (or locally isothermal
  disks) and for $\tilde{\beta}\sim 1$}.     
...
{\bf During the quasi-steady state the vortex 
  ...
  simultaneous with the appearence of the wakes, we observe the vortex
...
times. {\bf For completeness additional simultions were done for $\tilde\beta=0.01,0.25,0.75,2.5,7.5$. }
...
1$) the vortex lifetimes maximises at $\tilde\beta=2.5$ with 
