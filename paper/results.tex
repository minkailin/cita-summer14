...
\end{figure}
 ...
 For completeness we also simulated a locally
  isothermal disc with $h=0.05$ where the
...
{\bf This decay is probably due to numerical viscosity. During the
...
% (true?) (For isothermal and long
% cases yes. Spiral waves appear in beta=0.1,0.5,1.0}) 
We will see in the
...
\S\ref{lifetime_discuss}. Notice the decay timescale {\bf can be long} with
rapid cooling: for $\tilde{\beta}=0.1$ it takes $\sim400P_0$ 
...
, but becomes more compact approaching a ratio of 2 during its decay ($t=1310P_0$).   
...
{\bf During the quasi-steady state the vortex 
  ...
  % angular momentum to the disc (Is this true? Shocks on the inner disc
...
{\bf After full decay the aspect ratio at the outer gap edge is
  $h\sim0.05$. }
% (0.06 for beta=5.0,10)}

% what is aspect-ratio at outer gap edge after full decay?
...
times. For this study, additional simulations with 
...
decreases to $t_{\mathrm{life}} \approx 950P_0$ for
$\tilde{\beta}=0.25$. {\bf Note that for very small $\tilde{\beta}$, 
  there is significant contribution to the overall vortex lifetime due
  to a long decay timescale. }
For longer cooling times ($\tilde{\beta}\gtrsim
1$) the vortex lifetimes maximizes at $\tilde\beta=2.5$ with 
$t_{\mathrm{life}}\approx 1650P_0$.  

 We comment here that a locally isothermal simulation, where the
 ...
 equation with rapid cooling (or setting $\gamma$ close to 
 ...
 lifetime in a locally isothermal disc would 
...
In the previous section, we observed that vortices began to decay when it starts to
induce shocks. {\bf We thus suggest that the time needed for the vortex to grow to 
sufficient amplitude to induce shocks in the surrounding fluid,   
%The former timescale
%($t_\mathrm{diss}$), 
which may be considered as the duration of the quasi-steady state or
$t_\mathrm{diss}$, is an important contribution to the overall vortex
lifetime.} 
%reaches a single maximum with
%respect to cooling time {\bf for $\tilde{\beta}\geq 0.1$ (see also Fig. \ref{lifetimeplot})}. 
%{\bf (The case with $\tilde{\beta}=0.01$ and the locally isothermal disc, 
%does not follow this broad trend and may need to be analyzed separately.)} 
%The sum of these two timescales leads to the 
%double-peaked behaviour of $t_\mathrm{life}$ in Fig. \ref{betaplot}. 
We discuss below some competing factors that may result in a 
