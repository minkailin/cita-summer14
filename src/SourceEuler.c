...
void DensityPerturbation(Rho)
        PolarGrid *Rho;
{
        double mmax = (double) (PERT_MMAX - PERT_MMIN + 1);
        double rp = 10.0;
        int i, j, l, nr, ns, m;
        real rnd, sum, phi, dphi;
        real *rho;
        nr = Rho -> Nrad;
        ns = Rho -> Nsec;
        rho = Rho -> Field;
        srand((unsigned) (CPU_Rank+1)*time(0));
        dphi = 2.0*PI/(double)ns;
#pragma omp parallel for private(j,l) 
  for (i = 0; i < nr; i++) {
    if(Rmed[i]>rp) 
    ...
    for (j = 0; j < ns; j++) {
      ...
      phi = dphi*(double)j;
      sum = 0.0;
      for(m=PERT_MMIN; m<=PERT_MMAX; m++) sum+=cos(phi*(double)m); 
        rho[l] *= 1.0 + rnd*sum/(double)mmax;
    }
  }
}
     ...
     PlanetarySystem *sys;
{
  ...
  while (dtemp < 0.999999999*DT) {
    ...
    if( (PhysicalTime < PLANET_ON*2.0*M_PI) || (PhysicalTime > (PLANET_OFF)*2.0*M_PI)  ){
	MassTaper = 0.0;
    } else if ( PhysicalTime > PLANET_ON*2.0*M_PI && PhysicalTime < (PLANET_ON+MASSTAPER)*2.0*M_PI ){
    MassTaper = (PhysicalTime-PLANET_ON*2.0*M_PI)/(MASSTAPER*2.0*M_PI);
    MassTaper =  pow(sin(MassTaper*M_PI/2.0),2.0);
    } else {//if ( PhysicalTime > PLANET_OFF*2.0*M_PI && PhysicalTime < (PLANET_OFF+MASSTAPER)*2.0*M_PI ){
//    MassTaper = (PhysicalTime-PLANET_OFF*2.0*M_PI)/(MASSTAPER*2.0*M_PI);   
//    MassTaper =  1.0 - pow(sin(MassTaper*M_PI/2.0),2.0);
//    } else {
    MassTaper = 1.0;
    }
...
//          num = EnergyMed[i]*dt + CoolingTimeMed[i]*energy[l] + dt*CoolingTimeMed[i]*(qplus[l]-QplusMed[i]);
